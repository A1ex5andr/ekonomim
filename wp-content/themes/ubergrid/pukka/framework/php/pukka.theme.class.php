...
if( !class_exists('PukkaTheme') ) :
	...
	class PukkaTheme {
		...
		public function enqueueAdminScripts($hook) {
			...
			// icomoon
			wp_register_style('pukka-icomoon', PUKKA_FRAMEWORK_URI . '/css/icomoon/style.css', array());
			...
			// Used for nav tabs on Theme settings page
		...
		*/
		...
		private function fetchGoogleFonts() {
			...
			$title_font_effect = pukka_get_option('title_font_effect');
			$text_font = pukka_get_option('text_font');
			$text_font_effect = pukka_get_option('text_font_effect');
			$effects = "\n";

			if (!empty($title_font) && $title_font != 'default') {
				$url = '//fonts.googleapis.com/css?family=' . $title_font;
				if (!empty($title_font_effect) && $title_font_effect != 'none') {
					$url .= '&effect=' . $title_font_effect;

					$effects .= "\n var titleEffect = true;\n var titleEffectName = '" . $title_font_effect . "';";
				} else {
					$effects .= "\n var titleEffect = false;";
				}
				
				if('on' == pukka_get_option('enable_cyrillic_char_set')){
			...
			if (!empty($text_font) && $text_font != 'default') {
				$url = '//fonts.googleapis.com/css?family=' . $text_font;
				if (!empty($text_font_effect) && $text_font_effect != 'none') {
					$url .= '&effect=' . $text_font_effect;
					$effects .= "\n var textEffect = true;\n var textEffectName = '" . $text_font_effect . "';";
				} else {
		...
		/**
		 * Adds theme style css to be printed in page head
		 *
		 * @since Pukka 1.2.2
		 *
		 */	
		...
		/**
		 * Sets theme options from selected theme styles		 
		 *
		 * @since Pukka 1.2.2
		 *
		 * @param string $theme_style Theme settings key
		 * @param array $theme_options All theme options
		 * @return array Changed theme options 
		 *
		 */			
		...
		function deleteRetinaSupportImages( $attachment_id ) {
			...
			// Occurs when thumb generating fails (for example: timeout error during import)
			if( !is_array($meta) ){
				return;
			}
