	...
	class DynamicMetaTabs implements DMInterface {
		...
		public function getInputHTML($data = '') {
			...
			if(!empty($data['content'])){
				...
				$elem = new stdClass();
				$elem->text_content = '';
				$elem->text_color = '#000000';
				$elem->bg_color = '#ffffff';
				$elem->tab_title = '';
				$content->data[] = $elem;
				$content->data[] = $elem;
			}
			...
			foreach($content->data as $elem) :
								...
								$tabs .= '">
									<input type="text" placeholder="' . __('Enter title here', 'pukka') . '" class="dm-title dm-data-input" value="' . esc_attr($elem->tab_title) . '" data-var="tab_title">
											<div class="dm-remove-tab" title="Remove Tab">x</div>
								...
								$body .= '<li>
									<div class="dm-content-box">    
										...
										<div class="dm-input-tools">
											...
											<input type="hidden" class="dm-data-input dm-tabs-bg-color" value="' . esc_attr($elem->bg_color) . '" data-var="bg_color" />
		...
		public function getOutputHTML($data) {
		   ...
		   foreach($content->data as $elem) :
				...
				$tabs .= '" style="background-color: ' . esc_attr($elem->bg_color) . ';
					width: ' . $width . '%"><h4 style="color: ' . esc_attr($elem->text_color) . ';">' . $tab_title . '</h4></li>';
				
				$body .= '<li ';
				if(0 == $cnt){
					$body .= 'class="current" ';
				}
				$body .= 'style="background-color: ' . esc_attr($elem->bg_color) . '; 
					color: ' . esc_attr($elem->text_color) . '">                                
						' . $text_content . '
				</li>';
				$cnt++;
			endforeach;
